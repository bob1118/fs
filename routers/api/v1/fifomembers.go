...
func GetFifomembers(c *gin.Context) {
	...
	if rtcode == ec.SUCCESS {
		if fifos, err := db.SelectFifomembersWithCondition(condition); err != nil {
...
func PostFifomember(c *gin.Context) {
	...
	if rtcode == ec.SUCCESS {
		...
		if rtfifos, err := db.InsertFifoMembers(fifomembers); err != nil {
...
func DeleteFifomember(c *gin.Context) {
	...
	if c.ContentType() != gin.MIMEJSON {
		...
		if uuid := c.Param("uuid"); len(uuid) == 0 {
			...
			{
				if rtfifo, err := db.DeleteFifomembersFifomember(uuid); err != nil {
